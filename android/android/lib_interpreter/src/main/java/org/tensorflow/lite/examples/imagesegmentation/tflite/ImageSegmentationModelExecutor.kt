...
class ImageSegmentationModelExecutor(context: Context, private var useGPU: Boolean = false) {
  ...
  fun execute(data: Bitmap): ModelExecutionResult {
    try {
      ...
      val scaledBitmap = ImageUtils.scaleBitmapAndKeepRatio(data, reqWidthInPixels = width, reqHeightInPixels = height)
      //val scaledBitmap = Bitmap.createScaledBitmap(data, width, height, false)
      val contentArray =
        ImageUtils.bitmapToByteBuffer(scaledBitmap, width = width, height = height, mean = IMAGE_MEAN, std = IMAGE_STD)
      ...
      val emptyBitmap = ImageUtils.createEmptyBitmap(imageWidth = width, imageHeigth = height)
      return ModelExecutionResult(
        emptyBitmap,
        emptyBitmap,
        emptyBitmap,
  ...
  private fun convertBytebufferMaskToBitmap(
    ...
    val scaledBackgroundImage =
      ImageUtils.scaleBitmapAndKeepRatio(backgroundImage, imageHeight, imageWidth)
    ...
    for (y in 0 until imageHeight) {
      for (x in 0 until imageWidth) {
        ...
        for (c in 0 until NUM_CLASSES) {
          val value = inputBuffer.getFloat((y * imageWidth + x + c * imageHeight * imageWidth) * 4)
          if (c == 0 || value > maxVal) {
            maxVal = value
            mSegmentBits[x][y] = c
          }
  ...
  companion object {
    ...
    val labelsArrays =
      arrayOf(
        ...
        "sidewalk_damaged", "sidewalk_other", "sidewalk_soil_stone", "sidewalk_urethane"
      )
    //,
    //        "sidewalk_urethane"
