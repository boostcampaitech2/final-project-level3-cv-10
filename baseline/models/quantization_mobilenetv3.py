...
from models.mobilenetv3 import InvertedResidual, InvertedResidualConfig, MobileNetV3,\
    model_urls, _mobilenet_v3_conf
    ...
    def _load_from_state_dict(
        ...
        if version is None or version < 2:
            default_state_dict = {
                ...
                "scale_activation.activation_post_process.fake_quant_enabled":
                torch.tensor([1]),
                "scale_activation.activation_post_process.observer_enabled":
                torch.tensor([1]),
            }
...
def _load_weights(arch: str, model: QuantizableMobileNetV3,
                  model_url: Optional[str], progress: bool) -> None:
...
def _mobilenet_v3_model(
    ...
    **kwargs: Any,
    ...
    model = QuantizableMobileNetV3(inverted_residual_setting,
                                   last_channel,
    ...
    if quantize:
        ...
        if pretrained:
            _load_weights(arch, model,
                          quant_model_urls.get(arch + '_' + backend, None),
                          progress)
